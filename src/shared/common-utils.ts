...
export function getDurationFromMs(milliseconds: number | null | undefined): string | null | undefined {
    ...
    const minutes = Math.floor((milliseconds / Enum.Duration['One Minute']) % 60)
    const hours = Math.floor((milliseconds / Enum.Duration['One Hour']) % 24)
    const days = Math.floor(milliseconds / Enum.Duration['One Day'])
